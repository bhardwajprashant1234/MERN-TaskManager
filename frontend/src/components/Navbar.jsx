...
const Navbar = () => {
  ...
  return (
    <header className='flex justify-between sticky top-0 p-4 bg-gradient-to-r from-red-500 via-purple-500 via-green-500 via-orange-500 to-white-500 shadow-sm items-center'>
      <h2 className='cursor-pointer uppercase font-medium font-extrabold '>
        <Link to="/"> Task-hub </Link>
      </h2>
      
      <ul className='hidden md:flex gap-4 uppercase font-medium'>
        {authState.isLoggedIn ? (
          <>
            <li className="blue-500 text-black hover:bg-orange-400 font-medium rounded-md">
              <Link to='/tasks/add' className='block w-full h-full px-4 py-2'> <i className="fa-solid fa-plus"></i> Add task </Link>
            </li>
            <li className='py-2 px-3 cursor-pointer hover:bg-red-500 transition rounded-sm' onClick={handleLogoutClick}>Logout</li>
          </>
        ) : (
          <li className='py-2 px-3 cursor-pointer text-black hover:bg-cyan-200 transition rounded-sm'><Link to="/login">Login</Link></li>
